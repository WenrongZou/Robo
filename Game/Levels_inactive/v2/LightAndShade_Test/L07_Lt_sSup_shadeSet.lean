...
/- We don't need this level. -/

/-- The value of `f` at the supremum equals `f c`. -/
TheoremDoc lt_sSup_shadeSet as "fsSup_eq_fc" in "Shade"

Statement lt_sSup_shadeSet {f : ℝ → ℝ} {b c : ℝ} (hb : b ∉ Shade f)
    (hbc : f b < f c) (hc_shade : c ∈ Shade f) :
    c < sSup (shadeSet f c b) := by
  obtain ⟨x0, hx0⟩ := shadeSet_nonempty hb hbc hc_shade
  obtain hbd := shadeSet_bddAbove f c b
  obtain lt_aux := le_csSup hbd hx0
